...
contract AtomicSwapERC20 {
    ...
    modifier onlyInvalidSwaps(bytes32 _swapID) {
        console.log("Contract -->> onlyInvalidSwaps-States: States.INVALID", swapStates[_swapID] == States.INVALID);
        require(swapStates[_swapID] == States.INVALID);
        _;
    }

    modifier onlyOpenSwaps(bytes32 _swapID) {
        console.log('Contract -->> onlyOpenSwaps-States: States.OPEN', swapStates[_swapID] == States.OPEN);
        require(swapStates[_swapID] == States.OPEN);
        _;
    }

    modifier onlyClosedSwaps(bytes32 _swapID) {
        console.log("Contract -->> onlyClosedSwaps-States: States.CLOSED", swapStates[_swapID] == States.CLOSED);
        require(swapStates[_swapID] == States.CLOSED);
        _;
    }

    modifier onlyExpirableSwaps(bytes32 _swapID) {
        console.log("Contract -->> onlyExpirableSwaps-States: timelock ", swaps[_swapID].timelock <= now);
        require(swaps[_swapID].timelock <= now);
        _;
    }

    modifier onlyWithSecretKey(bytes32 _swapID, bytes memory _secretKey) {
        console.log("Contract -->> onlyWithSecretKey-States: States.CLOSED", swaps[_swapID].secretLock == sha256(_secretKey));
        bytes32 _key = sha256(_secretKey);
        console.log("Contract -->> onlyWithSecretKey _key: ");
    ...
    function open(bytes32 _swapID, uint256 _erc20Value, address _erc20ContractAddress, address _withdrawTrader,
        ...
        console.log("Contract -->> open(): States.OPEN ", swapStates[_swapID] == States.OPEN);
    ...
    function close(bytes32 _swapID, bytes memory _secretKey) public onlyOpenSwaps(_swapID)
        ...
        console.log("Contract -->> colse(): States.CLOSED ", swapStates[_swapID] == States.CLOSED);
    ...
    function expire(bytes32 _swapID) public onlyOpenSwaps(_swapID) onlyExpirableSwaps(_swapID) {
        ...
        console.log("Contract -->> expire(): States.EXPIRED ", swapStates[_swapID] == States.EXPIRED);
        emit Expire(_swapID);
    }
    ...
    function checkSecretKey(bytes32 _swapID) public view onlyClosedSwaps(_swapID) returns (bytes memory secretKey) {
        console.log("Contract -->> checkSecretKey():", swapStates[_swapID] == States.OPEN);
        Swap memory swap = swaps[_swapID];
